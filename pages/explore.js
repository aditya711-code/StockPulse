...
import { fetchTopGainersLosers,updateLoading,updatePricePercentage,updateType} from '@/redux/features/gainerslosersSlice';
...
const Explore=()=>{
    ...
    const { data, loading, error ,price,change_percentage,type} = useSelector((state) => state.topGainersLosers);
    ...
    const handleClick=(product)=>{
      ...
      if(price!=p && change_percentage!=change_p)
      {
        ...
        dispatch(updatePricePercentage({price:p,change_percentage:change_p}))
      }
              ...
              <Ripple/>
                <div className="w-10 p-1 border-1 surface-border surface-card border-square sm:w-20 sm:h-20">
                  ...
                  <div className="flex align-items-center justify-content-between">
                    ...
                    <span className="text-lg font-semibold " style={{color:layout==='gainers'?'green':'red'}}>
                      {layout=='gainers'?"+"+(parseFloat(product.change_percentage)).toFixed(2)+"%":""+(parseFloat(product.change_percentage)).toFixed(2)+"%"}
